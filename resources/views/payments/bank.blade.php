...
@section('content')
	...
	<form action='https://paycenter.piraeusbank.gr/redirection/pay.aspx' method='post' name='frm' target="pay">
		...
		<input name="MerchantId" type="hidden" value="{{$config['ONLINE_ACQUIRER_ID']}}" /> 
		<input name="PosId" type="hidden" value="{{$config['ONLINE_ACQUIRER_ID']}}" /> 
		<input name="User" type="hidden" value="{{$config['ONLINE_ACQUIRER_ID']}}" /> 
